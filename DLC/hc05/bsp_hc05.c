...
/**
  ...
  */
void
writeCommand(const char* command, const char* arg) {
    char str_buf[50];
...
/**
  ...
  */
uint8_t
HC05_Send_CMD(char* cmd, uint8_t clean) {
    uint8_t retry = 3;
    uint32_t i;
    uint16_t len;
    char* redata;
...
/**
  ...
  */
uint8_t
HC05_Send_CMD_Wait(char* cmd, uint8_t clean, uint32_t delayms) //Debug
{
    uint16_t len;
    char* redata;
...
/**
  ...
  */
char*
skipPrefix(char* str, size_t str_length, const char* prefix) {
...
/**
  ...
  */
void
rawReplaceChar(char* rawstr, char delimiter) {
    do {
