...
SPI1_Init(void) {
    ...
    SPI_InitStruct.SPI_Direction =
        SPI_Direction_2Lines_FullDuplex; // 设置SPI单向或双向的数据模式:SPI设置为双线双向全双工
    ...
    SPI_InitStruct.SPI_CPHA =
        SPI_CPHA_2Edge; // 针对SPI_CR1寄存器的CPHA位，串行同步时钟的第二个跳变沿（即上升沿）数据被采样
    SPI_InitStruct.SPI_NSS = SPI_NSS_Soft; // 针对SPI_CR1寄存器的SSM位，NSS信号由软件（使用SSI位）管理
    SPI_InitStruct.SPI_BaudRatePrescaler =
        SPI_BaudRatePrescaler_256; // 针对SPI_CR1寄存器的BR位，波特率预分频值为256，最低速率
...
************************************************************************/
void
SPI1_SetSpeed(u8 SPI_BaudRatePrescaler) {
